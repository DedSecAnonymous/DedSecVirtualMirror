package com.tastenkunst.as3.brf.nxt.examples.facesubstitution {
	 ...
	 */
	public class FaceSubstitutionCandideViewer extends ExampleBase {
		
		[Embed(source="assets/texture_marcel_candide.png")]
		public var IMAGE : Class;
		
		[Embed(source="assets/uv_marcel_candide.txt", mimeType="application/octet-stream")]
		...
		public const _outlineIndices : Vector.<int> = Vector.<int>([
			0, 11, 12, 14, 29, 28, 30, 32, 10, 65, 63, 61, 62, 47, 45, 44
		]);
		public const _outlinePoints : Vector.<Point> = new Vector.<Point>(_outlineIndices.length, true);
		...
		public function FaceSubstitutionCandideViewer(
...
//					DrawingUtils.drawTrianglesWithTexture(_draw, faceShape.faceShapeVertices, faceShape.faceShapeTriangles, _texture, _uvData);
					DrawingUtils.drawTrianglesWithTexture(_draw, faceShape.candideShapeVertices, faceShape.candideShapeTriangles, _texture, _uvData);
...
//			_drawMask.endFill();
		...
		public function calculateFaceOutline() : void {
			var candideVertices : Vector.<Number> = _brfManager.faceShape.candideShapeVertices;
			var center : Point = new Point(candideVertices[94 * 2], candideVertices[94 * 2 + 1]);
			...
			for (i = 0; i < l; i++) {
				var x : Number = candideVertices[_outlineIndices[i] * 2];
				var y : Number = candideVertices[_outlineIndices[i] * 2 + 1];
				
				tmpPointOutline = _outlinePoints[i];
				tmpPointOutline.x = x + (center.x - x) * fac;
				tmpPointOutline.y = y + (center.y - y) * fac;
			}
		}
